defmodule HexDocsSearchWeb.PackageLive.Index do
  ...
  defp search_results(assigns) do
    ...
    <ul class="-mb-2 py-2 text-sm text-gray-800 flex space-y-2 flex-col" id="options" role="listbox">
      ...
      <.link :for={{doc_id, doc} <- @docs} navigate={get_url(doc)} id={"doc-#{doc.id}"}>
  ...
  defp result_item(assigns) do
    ...
    <li
      id={@doc_id}
      class="cursor-default select-none px-4 py-2 text-xl bg-zinc-100 hover:bg-zinc-800 hover:text-white hover:cursor-pointer flex flex-row space-x-5 items-center"
      role="option"
      tabindex="-1"
    >
